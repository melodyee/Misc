...
module type CACHE = sig
  type t
  val create : float -> int -> int -> t (*size assoc*)
...
module CacheSet(C: CACHE) = struct
  type t = C.t array
  let create epsilon size assoc =
    assert (size mod assoc = 0);
    Array.init (size / assoc) (fun _ -> C.create epsilon assoc assoc)
  
end

module Sim (C: CACHE) = struct
  let trace_to_array trace = Array.map (fun e -> (0., e)) trace
  let hitRatio array result = sum
      (ExtArray.zipWith (fun (p, _) p' -> (1. -. p) *. p') array result)
    /. sum (Array.map (fun (p, _) -> 1. -. p) array)
  let runTraceProb epsilon size assoc array =
    let t = ref (C.create epsilon size assoc) in
    Array.init (Array.length array) (fun i ->
            let t', isHit = C.accept !t array.(i) in
            t := t';
            isHit)
  let runTrace epsilon size assoc trace =
    runTraceProb epsilon size assoc (trace_to_array trace)
  
  let runTraceMultiLevelProb epsilon size_array assoc trace =
    let t = ref (trace_to_array trace) in
    Array.map (fun size ->
            let res = runTraceProb epsilon size assoc !t in
            (* let hitRatio = hitRatio !t res in *)
            t := ExtArray.zipWith
              (fun (p, addr) p' -> 1. -. (1.-.p) *. (1. -. p'), addr) !t res;
            (* hitRatio, *)
            sum (Array.map fst !t),
            Array.map fst !t
        (* res *)
      ) size_array
  let runNTraceMultiLevelProb n epsilon size_array assoc trace =
    ExtArray.zipWith (fun a a' -> a, a')
      (average_array n (fun () ->
                Array.map fst (runTraceMultiLevelProb epsilon size_array assoc trace)))
      (average_array_array n (fun () ->
                Array.map snd (runTraceMultiLevelProb epsilon size_array assoc trace)))
  let runNTrace n epsilon size assoc trace =
    let caches = Array.init n (fun _ -> C.create epsilon size assoc) in
    Array.map (fun e ->
            average (Array.map (fun t -> let _, isHit = C.accept t e in isHit) caches))
      (trace_to_array trace)
  let runNTraceVariance n epsilon size assoc array =
    let arr = Array.init n
        (fun _ -> average (complement (runTrace epsilon size assoc array))) in
    average arr, sample_variance arr
  let runTraceApproxVariance lambdas consider_cov epsilon size assoc array =
    let arr = complement (runTrace epsilon size assoc array) in
  ...
  let runTraceApproxVarianceComplex _ _ epsilon size assoc array =
...
module MonteCarloCache = struct
  ...
  let zetas ?(total = false) size trace =
    ...
    let t = create epsilon size size in
    Array.map (fun addr ->
...
module ProbCache (K: KERNEL) = struct
  type t = {
    map : K.map;
    accept : address -> isHit;
...
module Historgram = struct
  ...
  let approx_hit_ratio t size scale =
    ...
    ExtList.take 10 (List.map (fun (a, b) -> a, 1. -. float b /. float scale)
...
module Lambda = struct
  ...
  let trace_to_lambdas epsilon size trace =
    ...
    let c = PC.create epsilon size size in
  ...
  let runTrace epsilon size _ trace =
...
let () =
  ...
  let testRunTrace ?(less = true) ?(print_trace = false) trace size =
    printf "testRunTrace\n";
    let aux f =
      let arr = (f epsilon size size trace) in
  ...
  let testSizeChange normalize trace =
    ...
    let aux f = fun epsilon size trace -> average (f epsilon size size trace) in
  ...
  let testRunTraceMultiLevel print_trace trace size =
    ...
    let aux f =
      let arr = (f epsilon sizes size trace) in
      if print_trace then begin
  ...
  let testRunTraceHistogram trace size =
    ...
    let reference = List.hd runTracesLessLess size size trace in
    let zero _ _ array = Array.map (fun _ -> 0.) array in
    let aux f =
      let n' = float (Array.length reference) in
      List.iter (fun (e, i) -> printf "%f " (float i /.n'))
        (Historgram.diff 100. reference (f size size trace));
      print_newline () in
  ...
  let testVariance trace size =
    ...
    let aux f =
      let miu, var = f epsilon size size trace in
      let std = var ** 0.5 in
      printf "miu = %f, miu-miu^2 = %f, std = %f, 3*std = %f\n"
        miu (miu -. miu ** 2.) std (3. *. std) in
    let test_dxi epsilon size assoc trace =
      let arr = complement (SM.runNTrace 500 epsilon size assoc trace) in
      average arr,
  ...
  let test_lambda print_lambdas trace size =
    ...
    if print_lambdas then begin
      ...
      let reference = SPB.runTrace epsilon size size trace in
      List.iter (fun arr ->
  ...
  let inc_variances size trace =
    ...
    Array.iter (printf "%f,")
      (Array.map (fun hit -> hit -. hit *.hit) (SM.runNTrace 500 epsilon size size trace));
    printf "\n";
    Array.iter (printf "%f,")
      (difference
...
(* Array.iter (testSizeChange true) traces; testRunTraceHistogram          *)
...
(* (trace_of_file 7 "var.trace"))); testRunTrace ~print_trace: true ~less: *)
(* true traces.(1) 4; testRunTrace ~print_trace: false ~less: true         *)
(* traces.(1) 4; testVariance traces.(1) 4; testRunTraceMultiLevel true;   *)
